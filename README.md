...
1. We provide data we downloaded in early November of 2025 and the plots generated from that data in `data/results`. Thus, you don't need to run any code in order to browse the data. Data includes
    1. Article metadata including citation count for all articles in journals Scientific Reports, Nature Communications and BMC Public Health.
    2. Generated figures and citation count ranks.
    

2. We provide code which was used to generate and analyze this data.
    1. `main_collect_articles.py` was used to get metadata (saved to the YYYY folders), for journal specified in `config.py`.
    2. `main_analysis.py` generates individual plots, for journal specified in `config.py`
